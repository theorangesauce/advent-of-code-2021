...
void next_cycle(std::map<int, int>& lanternfish_cycles);
void iterate(std::map<int, int>& lanternfish_cycles, int num_cycles);
int get_total_lanternfish(std::map<int, int>& lanternfish_cycles);
void print_cycles(std::map<int, int>& lanternfish_cycles);
  
int main() {
  std::string input;
  std::map<int, int> lanternfish_cycles;

  std::getline(std::cin, input);
  
  for (int i = 0; i <= 8; i++) {
    lanternfish_cycles.insert(std::pair<int, int>(i, 0));
  }
  ...
  iterate(lanternfish_cycles, 80);

  std::cout << "The number of lanternfish after 80 days is "
	    << get_total_lanternfish(lanternfish_cycles) << std::endl;
  return 0;
}

void next_cycle(std::map<int, int>& lanternfish_cycles) {
  int count_new_lanternfish = lanternfish_cycles.at(0);

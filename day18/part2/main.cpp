...
int main() {
  ...
  for (auto iter = input.begin(); iter != input.end(); ++iter) {
    numbers.push_back(SnailfishNumber(*iter));
  }

  int max_magnitude = 0;
  
  for (auto iter = numbers.begin(); iter != std::prev(numbers.end(), 1); ++iter) {
    for (auto sub_iter = std::next(iter, 1); sub_iter != numbers.end(); ++sub_iter) {
      SnailfishNumber result = *iter + *sub_iter;
      result.reduce();
      
      int magnitude = result.get_magnitude();
      
      if (magnitude > max_magnitude) {
	max_magnitude = magnitude;
      }

      result = *sub_iter + *iter;
      result.reduce();
      magnitude = result.get_magnitude();
  ...
  std::cout << "The maximum magnitude for any two snailfish numbers is " << max_magnitude << std::endl; 

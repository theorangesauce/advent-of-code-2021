...
std::vector<BingoBoard> get_boards(std::vector<std::string> input);
...
int main() {
  ...
  std::vector<BingoBoard> boards = get_boards(input);
  ...
  for (auto list_iter = number_list.begin(); list_iter != number_list.end(); ++list_iter) {
	...
	board_won = board_iter->get_board_id();
	break;
      }
    }
  ...
  int sum_unmarked_squares = boards.at(board_won).get_unmarked_sum();
  
  std::cout << "Board " << board_won << " won!" << std::endl;

...
std::size_t get_manhattan_distance(Probe& probe_left, Probe& probe_right);
std::size_t get_max_manhattan_distance(std::vector<Probe>& probes);
...
int main() {
  ...
  std::size_t max_manhattan_distance = get_max_manhattan_distance(probes);

  std::cout << "The max manhattan distance between two probes is " << max_manhattan_distance << std::endl;
  return 0;
}

std::size_t get_manhattan_distance(Probe& probe_left, Probe& probe_right) {
  auto offset_left = probe_left.get_offset();
  auto offset_right = probe_right.get_offset();

  return (abs(offset_right[0] - offset_left[0])
	  + abs(offset_right[1] - offset_left[1])
	  + abs(offset_right[2] - offset_left[2]));
}

std::size_t get_max_manhattan_distance(std::vector<Probe>& probes) {
  std::size_t max_manhattan_distance = 0;
  
  for (auto iter_left = probes.begin(); iter_left != probes.end(); ++iter_left) {
    for (auto iter_right = std::next(iter_left); iter_right != probes.end(); ++iter_right) {
      std::size_t dist = get_manhattan_distance(*iter_left, *iter_right);
